...
const innerRadiusScale = computed(() => 0.71);
const count = computed(() => Math.floor(max / 2) - min + 1)
...
function handScale(value: number) {
    return isInner(value) ? innerRadiusScale.value : 1.05;
}

function getPosition(value: number) {
    const rotateRadians = rotate * Math.PI / 180
    return {
        x: Math.sin((value - min) * degrees.value + rotateRadians) * handScale(value),
        y: -Math.cos((value - min) * degrees.value + rotateRadians) * handScale(value),
    }
}
...
function onDragMove(e: MouseEvent | TouchEvent) {
    ...
    const insideClick = euclidean(center, coords) < (innerWidth as number + innerWidth * innerRadiusScale.value) / 4
...
<template>
    <div class="pine-timer-hours" @mousedown="onMouseDown" @onTouchstart="onMouseDown" ref="clockRef">
        <div class="pine-timer-inner" ref="innerClockRef">
            ...
            <div v-for="n in genChildren" :key="n" :class="[`clock clock-${n}`, { 'intern': n > 11 }]"
                :style="[getTransform(n)]">
                {{ showValue(n) }}
            </div>
        </div>
    </div>
