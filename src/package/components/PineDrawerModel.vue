...
<template>
    <div class="pine-drawer-model">
        <div>
            <div class="header">
                <PineIcon name="XMark" color="white" :size="30" class="icon" @click="emit('clickOnClose')"></PineIcon>
            ...
            <div class="base">
                <div @click="item.disabled ? null : selectOption(ind)" v-for="(item, ind) in props.itens" :key="ind"
                    class="item-list"
                    :class="{ 'selected': internalModel === ind, 'right-icon': iconDirection == 'right', disabled: item.disabled }">
                    <PineIcon v-if="item.icon && props.showIcons && iconDirection == 'left'" :size="26" :name="item.icon"
                        color="white" class="icon">
                    </PineIcon>
                    <p class="text-item">
                        {{ item.title }}
                    </p>
                    <PineIcon v-if="item.icon && props.showIcons && iconDirection == 'right'" :size="26" :name="item.icon"
                        color="white" class="icon">
                    </PineIcon>
                </div>
            </div>
        ...
        <div class="end">
            <div class="item-list" :class="{ 'center-end': !props.showIcons, 'right-icon': iconDirection == 'right' }"
                @click="emit('clickOnLastItem')" v-if="lastOption">
                <PineIcon v-if="props.showIcons && lastOption?.icon && iconDirection == 'left'" :size="26"
                    :name="lastOption.icon" :color="lastOption.color" class="icon">
                </PineIcon>
                <p class="text-item">
                    {{ lastOption?.title }}
                </p>
                <PineIcon v-if="props.showIcons && lastOption?.icon && iconDirection == 'right'" :size="26"
                    :name="lastOption.icon" :color="lastOption.color" class="icon">
                </PineIcon>
            </div>
        </div>
